...
function candidatePool(p) {
  const pool = (MATCH_POOL[p.tur] || []).filter(
    (c) => c.cinsiyet !== p.cinsiyet,
  );
  return pool.length ? pool : MATCH_POOL[p.tur] || [];
}

export function updateMatchBadge() {
  const b = document.getElementById("hizMatchBadge");
  if (!b) return;
  const n = candidatePool(petData[appState.curPet]).filter(
    (c) => c.uyum >= 85,
  ).length;
...
function renderMatchCard() {
  const host = document.getElementById("mStack");
  if (!host) return;
  if (matchIdx >= matchPool.length) {
    host.innerHTML =
      '<div class="m-empty">Şimdilik aday kalmadı! 🐾<br><span style="font-size:12px;font-weight:600">Yeni profiller eklendikçe haber veririz.</span><a onclick="openMatch()">↻ Baştan göster</a></div>';
    return;
  }
  const c = matchPool[matchIdx];
  host.innerHTML =
    '<div class="m-card" id="mCard">' +
    '<div class="m-img"><img src="' +
    c.foto +
    '" alt="' +
    c.ad +
    '"><span class="m-uyum">💘 Uyum %' +
    c.uyum +
    "</span></div>" +
    '<div class="m-info"><b>' +
    c.ad +
...
function renderMatchStrip() {
  const s = document.getElementById("matchStrip");
  if (!s) return;
  s.innerHTML = matches.length
    ? '<span style="font-size:11px;color:var(--gray);font-weight:700;align-self:center;flex-shrink:0">Eşleşmeler:</span>' +
      matches
        .map(
          (c) =>
            '<div class="chip on" style="pointer-events:none">' +
            c.e +
            " " +
            c.ad +
            "</div>",
