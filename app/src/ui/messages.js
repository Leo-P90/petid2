...
export function startConversation(c) {
  ...
  conversations.set(c.ad, {
    candidate: c,
    messages: [{ from: "them", text: openerFor(c) }],
    unread: true,
  });
  renderConvoList();
  updateMsgBadge();
...
export function renderConvoList() {
  const host = el("msgList");
  if (!host) return;
  const list = [...conversations.values()];
  if (!list.length) {
    host.innerHTML =
      '<div class="m-empty" style="height:auto;padding:44px 20px;margin-top:10px">Henüz mesajın yok 💬<br><span style="font-size:12px;font-weight:600">Bir eşleşme olduğunda sohbet burada başlar.</span></div>';
    return;
  }
  host.innerHTML = list
    .map((conv) => {
      ...
      return (
        '<div class="msg-row" onclick="openThread(\'' +
        conv.candidate.ad +
        "')\">" +
        ...
        conv.candidate.ad +
        '"></div>' +
        '<div class="msg-body"><b>' +
        escapeHtml(conv.candidate.ad) +
        "</b><span>" +
...
export function openThread(ad) {
  const conv = conversations.get(ad);
  if (!conv) return;
  conv.unread = false;
  activeThread = ad;
  updateMsgBadge();
  renderConvoList();
  pID("threadName", "Demo sohbet — " + conv.candidate.ad);
  const img = el("threadAvatar");
  if (img) img.src = conv.candidate.foto;
