...
export function sagJump(id, chip) {
  ...
  if (!e || !e.hasAttribute("data-health-panel")) return;
  if (chip) {
  ...
  if (e) {
    document
      .querySelectorAll("#scr-saglik [data-health-panel]")
      .forEach((p) => (p.hidden = p !== e));
    e.hidden = false;
    document.getElementById("scr-saglik").scrollTop = 0;
  }
}
  ...
  if (willOpen && !detail.dataset.built) {
    ...
    detail.innerHTML = info
      ? '<div class="vac-detail-inner">' +
        '<div class="se-group"><b style="color:var(--green)">✅ Normal (beklenen)</b>' +
        renderSeList(info.normal) +
        "</div>" +
        '<div class="se-group"><b style="color:var(--red)">⚠️ Riskli (dikkat)</b>' +
        renderSeList(info.riskli) +
        "</div>" +
