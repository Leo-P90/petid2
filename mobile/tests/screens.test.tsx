...
test('report denial remains recoverable with manual address and local-only preview', async () => {
  ...
  await mount(<Reports />);
  expect(screen.getByText('1 · Kimin için?')).toBeTruthy();
  expect(screen.queryByText('2 · Ne oldu?')).toBeNull();
  await fireEvent.press(screen.getByRole('button', { name: 'Kendi hayvanım için' }));
  await fireEvent.press(screen.getByRole('button', { name: 'Mia · Kedi' }));
  await fireEvent.press(screen.getByRole('button', { name: 'Kayıp' }));
  expect(screen.getByText('3 · Bilgiler')).toBeTruthy();
  expect(nativeServices.locate).not.toHaveBeenCalled();
  await fireEvent.press(screen.getByRole('button', { name: 'Mevcut konumu kullan' }));
  await waitFor(() => expect(screen.getByText(/İzin verilmedi/)).toBeTruthy());
  await fireEvent.changeText(screen.getByLabelText('Haritadan / adresle seç'), 'Kadıköy park girişi');
  await fireEvent.press(screen.getByRole('button', { name: 'Seçilen konumu onayla' }));
  await fireEvent.press(screen.getByRole('button', { name: 'Fotoğraf seç' }));
  await waitFor(() => expect(screen.getByLabelText('Bildirim fotoğrafı 1')).toBeTruthy());
  await fireEvent.changeText(screen.getByLabelText('Durumu kısaca anlat'), 'Park girişinde son kez görüldü.');
  await fireEvent.changeText(screen.getByLabelText('Son görülme zamanı'), '2026-09-18T12:00');
  await fireEvent.press(screen.getByRole('button', { name: 'Bilgilere devam' }));
  expect(screen.getByText('4 · Kimler görsün?')).toBeTruthy();
  await fireEvent.press(screen.getByRole('button', { name: 'Bildirimi önizle' }));
  expect(screen.getByText('5 · Bildirim önizlemesi')).toBeTruthy();
  expect(screen.queryByText(/Gerçek ilan yayınlanmadı/)).toBeNull();
  jest.spyOn(AsyncStorage, 'setItem').mockRejectedValueOnce(new Error('storage full'));
  await fireEvent.press(screen.getByRole('button', { name: 'Bildirimi yayınla' }));
  await waitFor(() => expect(screen.getByText(/Yerel taslak kaydedilemedi/)).toBeTruthy());
  expect(screen.getByRole('button', { name: 'Bildirimi yayınla' }).props.accessibilityState.disabled).toBe(false);
  await fireEvent.press(screen.getByRole('button', { name: 'Bildirimi yayınla' }));
  await waitFor(() => expect(screen.getByText(/Gerçek ilan yayınlanmadı/)).toBeTruthy());
  jest.restoreAllMocks();
});
test('street report requires an explicit other-animal selection and one situation', async () => {
  jest.requireMock('expo-router').useLocalSearchParams.mockReturnValue({ source: 'street' });
  await mount(<Reports />);
  await fireEvent.press(screen.getByRole('button', { name: 'Başka bir hayvan için' }));
  expect(screen.queryByText('2 · Ne oldu?')).toBeNull();
  await fireEvent.press(screen.getByRole('button', { name: 'Köpek' }));
  expect(screen.getByText('2 · Ne oldu?')).toBeTruthy();
  await fireEvent.press(screen.getByRole('button', { name: 'Yardıma ihtiyacı var' }));
  expect(screen.getByText('3 · Bilgiler')).toBeTruthy();
  expect(screen.queryByText('4 · Kimler görsün?')).toBeNull();
  expect(nativeServices.locate).not.toHaveBeenCalled();
});
