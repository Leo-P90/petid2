...
export default function Home() {
  ...
  return <Screen title="PetID" tab><PetSelector />
    ...
    <Label heading>Yaklaşan bakım</Label><Card><Label>Bakım defteri seni bekliyor</Label><Note>Henüz kayıtlı bir bakım tarihi yok. Hatırlatma veya sağlık skoru hesaplanmıyor.</Note><RouteButton label="Bakım defterini aç" href="/health" /></Card>
    <Label heading>Birlikte daha güzel</Label><Pressable accessibilityRole="button" accessibilityLabel="PatiMatch keşfi" onPress={() => router.push('/match')} style={{ backgroundColor: colors.redSoft, padding: 18, borderRadius: 20, flexDirection: 'row', alignItems: 'center', gap: 12, minHeight: 96 }}><Text style={{ fontSize: 34 }}>💞</Text><View style={{ flex: 1 }}><Label>Yeni bir patiyle tanış</Label><Note>{pet.species} arkadaşlar · Karşılıklı demo beğeni</Note></View><Text style={{ color: colors.text }}>→</Text></Pressable>
