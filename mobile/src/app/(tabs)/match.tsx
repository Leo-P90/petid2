...
export function SwipeCard({ candidate, choose, reduced, live = false, undo, canUndo = false, info }: { candidate: Candidate; choose: (like: boolean) => void; reduced: boolean; live?: boolean; undo?: () => void; canUndo?: boolean; info?: () => void }) {
  ...
  // eslint-disable-next-line react-hooks/refs
  return <><Animated.View {...pan.panHandlers} testID="match-swipe-card" style={{ flex: 1, minHeight: 220, borderRadius: 28, overflow: 'hidden', transform: [{ translateX: x }, { rotate: reduced ? '0deg' : x.interpolate({ inputRange: [-width, 0, width], outputRange: ['-14deg', '0deg', '14deg'] }) }] }}><Portrait candidate={candidate} live={live} /><LinearGradient pointerEvents="none" colors={['transparent', '#101116C0', '#101116FA']} locations={[0, .45, 1]} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20, paddingTop: 90, gap: 8 }}><View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}><Text accessibilityRole="header" style={{ fontSize: 30, fontWeight: '800', color: '#FFFFFF' }}>{candidate.name}</Text><Text style={{ color: '#FFFFFF', fontSize: 18 }}>{candidate.age}</Text><View accessibilityLabel={candidate.verification === 'verified' ? 'Server onaylı profil' : 'Profil onayı henüz yok'} style={{ borderRadius: 12, paddingHorizontal: 7, paddingVertical: 3, backgroundColor: candidate.verification === 'verified' ? '#16A36A30' : '#FFFFFF18' }}><Text style={{ color: candidate.verification === 'verified' ? '#96E7BD' : '#CFCDD7', fontSize: 11 }}>{candidate.verification === 'verified' ? '✓' : candidate.verification === 'pending' ? '◷' : '—'}</Text></View></View><Text numberOfLines={2} style={{ color: '#E3DFF0', fontSize: 13, lineHeight: 20 }}>{candidate.species} · {candidate.breed} · {candidate.gender}  ·  {candidate.distance}</Text></LinearGradient></Animated.View><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-evenly', paddingVertical: 12 }}>{[{ label: 'Geri al', symbol: '↶', action: undo, disabled: !canUndo, color: colors.accent, size: 58 }, { label: 'Geç', symbol: '×', action: () => commit(false), color: '#FFFFFF', size: 62 }, { label: 'Aday bilgisi', symbol: 'i', action: info, color: colors.accent, size: 58 }, { label: live ? 'Beğen' : 'Beğen · demo', symbol: '♥︎', action: () => commit(true), color: '#FFFFFF', size: 64 }].map(item => <Pressable key={item.label} accessibilityRole="button" accessibilityLabel={item.label} accessibilityState={{ disabled: busy || !!item.disabled || !item.action }} disabled={busy || !!item.disabled || !item.action} onPress={item.action} style={{ width: item.size, height: item.size, borderRadius: item.size / 2, alignItems: 'center', justifyContent: 'center', backgroundColor: item.label.startsWith('Beğen') ? '#F43F5E' : colors.surface, borderWidth: 1, borderColor: item.label.startsWith('Beğen') ? '#F43F5E' : colors.border, opacity: item.disabled ? .4 : 1 }}><Text style={{ fontSize: item.label.startsWith('Beğen') ? 35 : 28, color: item.color }}>{item.symbol}</Text></Pressable>)}</View></>;
}
