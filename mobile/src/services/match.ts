...
export function createMatchRepository(client: SupabaseClient | null) {
  ...
  return {
    async snapshot(petId: string): Promise<MatchSnapshot> {
      ...
      const [pool, likes, participants, media] = await Promise.all([
      ...
      ]); for (const result of [pool, likes, participants, media]) if (result.error) throw result.error;
      ...
      return { profile, candidates: ((pool.data ?? []) as MatchProfile[]).filter(other => eligibleCandidate(profile, other) && !likes.data?.some(like => like.liked_pet === other.pet_id)), threads: threads.filter(row => row !== null), media: (media.data ?? []) as MatchMedia[] };
    },
    ...
    async upload(petId: string, uri: string) {
      ...
      if (!ext || file.size < 1 || file.size > 5242880) throw new Error('JPEG, PNG veya WebP · en fazla 5 MB.');
      const object_name = `${secureRequestId()}.${ext}`; const storage = db().storage.from('match-media');
      const { data, error } = await db().from('match_media').insert({ pet_id: petId, object_name }).select('id').single(); if (error) throw error;
      const { error: uploadError } = await storage.upload(object_name, await file.arrayBuffer(), { contentType: file.type, upsert: false });
    ...
    async photo(petId: string): Promise<{ uri: string; dispose: () => void } | null> {
      ...
      file.create(); file.write(new Uint8Array(await result.data.arrayBuffer()));
